...
func runStakersTest(t *testing.T, faultConfig challenge.FaultConfig, maxGasPerNode *big.Int, expectedEnd ExpectedChallengeEnd) {
	...
	val, err := ethbridge.NewValidator(ethcommon.Address{}, validatorWalletFactory, rollupAddr, client, valAuth, nil)
	test.FailIfError(t, err)
	val2, err := ethbridge.NewValidator(ethcommon.Address{}, validatorWalletFactory, rollupAddr, client, val2Auth, nil)
	test.FailIfError(t, err)
