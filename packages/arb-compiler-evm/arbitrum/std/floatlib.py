        ...
        lambda vm: [
            ...
            vm.ifelse(
                lambda vm: [
                    ...
                    local_vars.get("mant"),
                    vm.ifelse(
                        lambda vm: [vm.push(1), local_vars.set_val("isnan")],
                        lambda vm: [vm.push(1), local_vars.set_val("isinf")],
                    ),
                ],
                lambda vm: [
                    # normal case
        ...
        lambda vm: [
            ...
            vm.ifelse(
                lambda vm: [
                    ...
                    # mant exp
                    vm.while_loop(
                        lambda vm: [vm.dup0(), vm.push(1 << _mantbits), vm.gt()],
                        lambda vm: [
                            vm.push(2),
                            vm.mul(),
                            vm.swap1(),
...
@modifies_stack(4, 1)
def pack(vm):
    local_vars = Locals(vm, ["sign", "mant", "mant0", "exp", "exp0", "trunc", "trunc0"])
    local_vars.new()
    ...
    local_vars.get("mant")
    vm.ifelse(
        lambda vm: [  # first for-loop in go code
            local_vars.get(["mant", "exp"]),
            vm.while_loop(
                lambda vm: [vm.push(1 << _mantbits), vm.dup1(), vm.slt()],
                lambda vm: [
                    # mant exp
                    vm.push(2),
                    vm.mul(),
            ...
            vm.while_loop(
                lambda vm: [vm.dup1(), vm.push(4 << _mantbits), vm.slt()],
                lambda vm: [
                    # trunc mant exp
                    vm.dup1(),
                    vm.push(1),
            ...
            vm.slt(),
            vm.ifelse(
                lambda vm: [  # in go code, second top-level if statement
                    # trunc mant exp
                    vm.dup1(),
                    vm.push(1),
                    vm.bitwise_and(),
            ...
            vm.slt(),
            vm.ifelse(
                lambda vm: [  # third top-level if statement in go code
                    vm.pop(),
                    vm.pop(),
                    vm.pop(),
                    local_vars.get("sign"),
                ...
                lambda vm: [
                    ...
                    vm.sgt(),
                    vm.ifelse(
                        lambda vm: [  # last top-level if statement in go code
                            # trunc mant exp
                            vm.pop(),
                            vm.pop(),
                            # exp
...
def add(vm):
    ...
    unpack(vm)
    tup.tbreak(5)(vm)
    local_vars.set_val(["fs", "fm", "fe", "fi", "fn", "gs", "gm", "ge", "gi", "gn"])

    local_vars.get(["fn", "gn"])
    vm.bitwise_or()
    vm.ifelse(
        lambda vm: [vm.push(_nan)],
        lambda vm: [
            ...
            vm.ifelse(
                ...
                lambda vm: [
                    local_vars.get("fi"),
                    vm.ifelse(
                        lambda vm: [local_vars.get("f")],  # third if in go code
                        lambda vm: [
                            local_vars.get("gi"),
                            vm.ifelse(
                                lambda vm: [  # fourth if in go code
                                    local_vars.get("g")
                                ],
                                lambda vm: [
...
def mul(vm):
    ...
    vm.ifelse(
        ...
        lambda vm: [
            ...
            vm.bitwise_and(),
            vm.ifelse(
                lambda vm: [local_vars.get(["f", "gs"]), vm.bitwise_xor()],
                lambda vm: [
...
def div(vm):
    ...
    vm.bitwise_or()
    vm.ifelse(
        lambda vm: [vm.push(_nan)],  # first two cases in go code
        lambda vm: [
                        ...
                        lambda vm: [
                            ...
                            vm.ifelse(
                                ...
                                lambda vm: [
                                    ...
                                    local_vars.get("fs"),
                                    vm.ifelse(lambda vm: [vm.push(0), vm.sub()]),
                                ],
                            ),
                        ],
                        lambda vm: [
