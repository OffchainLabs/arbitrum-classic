...
contract Rollup is Cloneable, RollupCore, Pausable, IRollup {
    ...
    function stakeOnNewNode(
        ...
        InboxAssertionType[2] calldata inboxAssertionTypes
    ) external whenNotPaused {
        require(isStaked(msg.sender), "NOT_STAKED");
        ...
        INode node;
        {
            INode prevNode = getNode(latestStakedNode(msg.sender));
            {
                ...
                {
                    uint256 timeSinceLastNode =
                        block.number.sub(assertion.beforeState.proposedBlock);
                    // Verify that assertion meets the minimum Delta time requirement
                    require(timeSinceLastNode >= minimumAssertionPeriod, "TIME_DELTA");

                    uint256 gasUsed =
                        assertion.afterState.gasUsed.sub(assertion.beforeState.gasUsed);
                    ...
                    require(
                        ...
                        (assertion.afterState.inboxCount >=
                            assertion.beforeState.inboxMaxCount[0] &&
     ...
     * @param maxMessagePeeks The number of messages in each inbox needed to determine the validity of each assertion
     * @param inboxesAssertedEmpty Whether each assertion asserted that each inbox was empty when it was proposed
     */
    function createChallenge(
        ...
        uint256[2][2] calldata maxMessagePeeks,
        bool[2][2] calldata inboxesAssertedEmpty
    ) external whenNotPaused {
