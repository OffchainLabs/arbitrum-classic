...
// CHRIS: should NodeFixed be a type or interface
...
async function makeSimpleNode(
  ...
  // CHRIS: should probably test the nodefixed data - here and elsewhere
...
const initNewRollup = async () => {
  ...
  const nFHash = await rollup.getNodeFixedHash(originalNode)
  ...
  const stateHash = await nodeStateHash({
    ...
    fixedData: {
      ...
      confirmData: zerobytes32,
      stateHash: zerobytes32,
    },
  })
  prevNode = {
    assertion: initialAssertion,
  ...
  assert.equal(
    ...
    nodeFixedHash(
      stateHash,
      prevNode.fixedData.challengeHash,
      prevNode.fixedData.confirmData
    ),
...
describe('ArbRollup', () => {
  ...
  it('should confirm node', async function () {
    ...
    const firstUnresolved = await rollup.rollup.firstUnresolvedNode()
    const node = await nodeNumToNode(firstUnresolved, rollup.rollup)
