...
function bisection(
  ...
  client
    .request("RPCInterface.GenerateBisection", [data])
    .then(function(response) {
      if (response.result.assertion.numSteps > 1) {
        var afterHashes = [];
        var totalSteps = 0;
        var messageBisections = [
          response.result.bisections[0].firstMessageHash
        ];
        var messageTotals = [];
        var messageCoinNums = [];
        ...
        for (var i = 0; i < response.result.bisections.length; i++) {
          ...
          messageCoinNums = messageCoinNums.concat(
            bisection.totalMessageCoinNums
          );
          messageCoinCount.push(bisection.totalMessageAmounts.length);
        }
        let afterHashesAndMessageBisections = afterHashes.concat(
          messageBisections
        );
        ...
        return challengeVerifier
          .bisectAssertion(
            [
              vmId,
              response.result.precondition.beforeHash,
              response.result.precondition.beforeInbox
            ],
...
export class RPCManager {
  constructor(
    ...
    for (var i = 0; i < config.assertKeys.length; i++) {
      let addressBytes = Buffer.concat([
        Buffer.alloc(12),
        Buffer.from(config.assertKeys[i].slice(2), "hex")
      ]);
    ...
    this.managerProof = tree.getProofOrdered(
      tree.elements[managerNum],
      managerNum + 1,
      true
    );
    ...
    client
      .request("RPCInterface.RegisterVM", [registerData])
    ...
    disputableAssertionWatcher.watch(function(err, result) {
      let data = {
        ...
        totalMessageValues: result.args.totalMessageValues.map(b =>
          b.toString(10)
        )
      };
      ...
      client
        .request("RPCInterface.SawDisputableAssertion", [data])
        .then(function(response) {
    ...
    const confirmedAssertionWatcher = vmTracker.ConfirmedAssertion({
      vmId: vmId
    });
    confirmedAssertionWatcher.watch(function(err, result) {
      if (self.managerState == ManagerState.PENDING) {
        ...
        client
          .request("RPCInterface.SawFinalizedAssertion", [data])
          .then(function(response) {
    ...
    const bisectionAssertionWatcher = challengeVerifier.BisectedAssertion({
      vmId: vmId
    });
    bisectionAssertionWatcher.watch(function(err, result) {
      ...
      let bisectionCount =
        (result.args.afterHashAndMessageBisections.length - 1) / 2;
      ...
      for (var i = 0; i < bisectionCount; i++) {
        ...
        bisections.push({
          ...
          numSteps: numSteps,
          firstMessageHash:
            result.args.afterHashAndMessageBisections[bisectionCount + i],
          lastMessageHash:
            result.args.afterHashAndMessageBisections[bisectionCount + i + 1],
          totalMessageAmounts: amounts.slice(count, count + coinCount),
          totalMessageCoinNums: result.args.totalMessageCoinNums.slice(
            count,
            count + coinCount
          )
        });
      ...
      client
        .request("RPCInterface.SawBisection", [data])
        .then(function(response) {
          ...
          for (var i = 0; i < response.result.bisections.length; i++) {
            ...
            hashes.push(
              utils.toBuffer(
                abi.soliditySHA3(
                  ["bytes32", "bytes32"],
                  [preconditionHash, assertionHash]
                )
              )
            );
          }
          ...
          let proof = tree.getProofOrdered(
            tree.elements[response.result.invalidBisection],
            response.result.invalidBisection + 1,
            true
          );
    ...
    const initiatedChallengeWatcher = challengeVerifier.InitiatedChallenge({
      vmId: vmId
    });
    ...
    const challengeWatcher = challengeVerifier.ContinuedChallenge({
      vmId: vmId
    });
    challengeWatcher.watch(function(err, result) {
      ...
      return bisection(
        ...
        vmId,
        challengeVerifier,
        result.args.assertionIndex.toNumber(),
        self.address,
        managerNum,
        config,
        result.blockNumber
  ...
  async disputableAssert(numSteps) {
    ...
    let data = {
      ...
      numSteps: numSteps,
      timeBounds: [web3.eth.blockNumber, web3.eth.blockNumber + 2].map(val =>
        val.toString()
      )
    };
    let response = await this.client.request(
      "RPCInterface.GenerateDisputableAssertion",
      [data]
    );
    var [
      messageData,
  ...
  getVMState(callback) {
    ...
    this.client
      .request("RPCInterface.GetVMState", [data])
      .then(function(response) {
        callback(response);
      });
  }
