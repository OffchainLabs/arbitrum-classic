...
TEST_CASE("CombinedSideloadCache add and get") {
    ...
    REQUIRE(cache.atOrBeforeGas(50, 0, 0, database_load_cost) == nullptr);
    ...
    auto machine41a = cache.atOrBeforeGas(50, 0, 0, 10000);
    ...
    auto machine42a = cache.atOrBeforeGas(50, 0, 0, 10000);
    ...
    auto machine43a = cache.atOrBeforeGas(50, 0, 0, 10000);
    ...
    auto machineDBa = cache.atOrBeforeGas(50, 0, 53, 10);
    ...
    auto machineDBb = cache.atOrBeforeGas(50, 0, 54, 10);
    REQUIRE(machineDBb == nullptr);

    // Test current machine closer than database load
    auto machineDBc = cache.atOrBeforeGas(50, 53, 53, 10);
    ...
    machine42->machine_state.output.arb_gas_used = 42;
    cache.lru_add(std::move(machine42));
    machine42a = cache.atOrBeforeGas(50, 0, 0, 10000);
    REQUIRE(machine42a != nullptr);
    ...
    machine43->machine_state.output.last_inbox_timestamp = std::time(nullptr);
    cache.timed_add(std::move(machine43));
    machine43a = cache.atOrBeforeGas(50, 0, 0, 10000);
    REQUIRE(machine43a != nullptr);
}
