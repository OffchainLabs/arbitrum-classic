...
type ValidatorProxy interface {
	...
	CallMessage(contract common.Address, sender common.Address, data value.Value) (value.Value, error)
}
...
func (vp *ValidatorProxyImpl) CallMessage(contract common.Address, sender common.Address, data value.Value) (value.Value, error) {
	var buf bytes.Buffer
