...
const main = async () => {
  ...
  const inboxEvent = deployReceipt.logs.filter(
    log => log.topics[0] === inboxEventSignature[0] || log.topics[0] === inboxEventSignature[1]
  )
  ...
  const l2Provider = new ethers.providers.JsonRpcProvider("https://devnet-l2.arbitrum.io/rpc")
  const l2TxReceipt = await l2Provider.getTransactionReceipt(l2DeployTxHash)

  const l2ToL1EventId = id('L2ToL1Transaction(address,address,uint256,uint256,uint256,uint256,uint256,uint256,uint256,bytes)')
  const logs = l2TxReceipt.logs.filter(log => log.topics[0] === l2ToL1EventId)
  
  if(logs.length !== 1) throw new Error("Not exactly 1 log emitted of L2 to L1 tx?")
  ...
  const nodeInterface = new ethers.Contract(nodeInterfaceAddress, foo).connect(l2Provider);
