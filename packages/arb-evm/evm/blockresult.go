...
func parseGasAccountingSummary(val value.Value) (*GasAccountingSummary, error) {
	tup, ok := val.(*value.TupleValue)
	if !ok {
		return nil, errors.Errorf("expected gas accounting summary to be tuple of length 7")
	}
	startIndex := int64(0)
	if tup.Len() == 7 {
		startIndex = 1
	} else if tup.Len() != 6 {
		return nil, errors.Errorf("expected gas accounting summary to be tuple of length 6 or 7, not %v", tup.Len())
	}

	// Tuple size already verified above, so error can be ignored
	pricePerL1CalldataByte, _ := tup.GetByInt64(startIndex + 0)
	pricePerStorageCell, _ := tup.GetByInt64(startIndex + 1)
	pricePerArbGasBase, _ := tup.GetByInt64(startIndex + 2)
	pricePerArbGasCongestion, _ := tup.GetByInt64(startIndex + 3)
	pricePerArbGasTotal, _ := tup.GetByInt64(startIndex + 4)
	gasPool, _ := tup.GetByInt64(startIndex + 5)
