import {
  ...
  describe('Bridge Specific', () => {
    ...
    it('should not be able to redeem after the retryable has been cancelled', async () => { 
        ...
        const depositRes = await bridge.deposit(existentTestERC20, tokenDepositAmount, {maxGas: BigNumber.from(0)}) 
        ...
        const retryableTicket = await testRetryableTicketNoAutoRedeem(bridge, depositRec)   
        console.log("here")
    ...
    it('should not have anything in the retry buffer after cancelling', async() => {
        ...
        const timeoutBefore = await bridge.l2Bridge.arbRetryableTx.getTimeout(depositTx.retryableTicket)  
        ...
        //check retry buffer for ticket 
        const timeout = await bridge.l2Bridge.arbRetryableTx.getTimeout(depositTx.retryableTicket)  
      ...
      it('beneficiary should have call value in funds after cancellation', async () => {
        ...
        const beneficiaryAddr = await bridge.l2Bridge.arbRetryableTx.getBeneficiary(bridgeTxn.retryableTicket)
        const beneficiaryBalance = bridge.l2Provider.getBalance(beneficiaryAddr) 
    
        expect(beneficiaryBalance).to.equal(.001) //check against l1 fund amount
      }) 
