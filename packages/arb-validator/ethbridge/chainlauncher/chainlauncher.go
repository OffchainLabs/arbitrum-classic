...
// ArbChainPendingAssertionCanceledIterator is returned from FilterPendingAssertionCanceled and is used to iterate over the raw logs and unpacked data for PendingAssertionCanceled events raised by the ArbChain contract.
type ArbChainPendingAssertionCanceledIterator struct {
	Event *ArbChainPendingAssertionCanceled // Event containing the contract specifics and raw log
...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *ArbChainPendingAssertionCanceledIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	...
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(ArbChainPendingAssertionCanceled)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *ArbChainPendingAssertionCanceledIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *ArbChainPendingAssertionCanceledIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// ArbChainPendingAssertionCanceled represents a PendingAssertionCanceled event raised by the ArbChain contract.
type ArbChainPendingAssertionCanceled struct {
	Raw types.Log // Blockchain specific contextual infos
}
...
func (_ArbChain *ArbChainFilterer) FilterPendingAssertionCanceled(opts *bind.FilterOpts) (*ArbChainPendingAssertionCanceledIterator, error) {

	logs, sub, err := _ArbChain.contract.FilterLogs(opts, "PendingAssertionCanceled")
	if err != nil {
		return nil, err
	}
	return &ArbChainPendingAssertionCanceledIterator{contract: _ArbChain.contract, event: "PendingAssertionCanceled", logs: logs, sub: sub}, nil
}
...
//
// Solidity: event PendingAssertionCanceled()
func (_ArbChain *ArbChainFilterer) WatchPendingAssertionCanceled(opts *bind.WatchOpts, sink chan<- *ArbChainPendingAssertionCanceled) (event.Subscription, error) {

	logs, sub, err := _ArbChain.contract.WatchLogs(opts, "PendingAssertionCanceled")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(ArbChainPendingAssertionCanceled)
				if err := _ArbChain.contract.UnpackLog(event, "PendingAssertionCanceled", log); err != nil {
					return err
				}
				event.Raw = log
...
//
// Solidity: event PendingAssertionCanceled()
func (_ArbChain *ArbChainFilterer) ParsePendingAssertionCanceled(log types.Log) (*ArbChainPendingAssertionCanceled, error) {
	event := new(ArbChainPendingAssertionCanceled)
	if err := _ArbChain.contract.UnpackLog(event, "PendingAssertionCanceled", log); err != nil {
		return nil, err
	}
