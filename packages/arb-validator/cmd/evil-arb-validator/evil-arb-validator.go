...
func createRollupChain() {
	...
	flag.Parse()
	if flag.NArg() != 5 {
		log.Fatalln("usage: evil-arb-validator create <contract.ao> <private_key.txt> <ethURL> <bridge_eth_addresses.json>")
	}
...
func validateRollupChain() error {
	...
	if validateCmd.NArg() != 3 {
		return errors.New("usage: evil-arb-validator validate [--password=pass] [--rpc] [--blocktime=NumSeconds] [--gasprice==FloatInGwei] <validator_folder> <ethURL> <rollup_address>")
	}
