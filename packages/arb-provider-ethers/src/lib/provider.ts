...
export class ArbProvider extends ethers.providers.BaseProvider {
    ...
    public async getMessageResult(
        ...
        if (
            !this.processLogsProof(val, logPreHash, logPostHash, logValHashes)
        ) {
        ...
        if (validatorSigs && validatorSigs.length > 0) {
            ...
            this.processConfirmedDisputableAssertion(
                logPostHash,
                onChainTxHash
            );
        }
    ...
    private async processConfirmedDisputableAssertion(
        ...
        const eventIndex = events.findIndex(
            event => event.name === EB_EVENT_CDA
        );
        if (eventIndex == -1) {
            throw Error(
                'RollupAsserted ' + onChainTxHash + ' not found on chain'
            );
        }
