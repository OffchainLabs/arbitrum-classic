...
describe('Constructors', function() {
    ...
    test('ImmOp', function() {
        const iop = new arb.ImmOp(
            0x19,
            new arb.IntValue(utils.bigNumberify(9))
        );
...
describe('Marshaling', function() {
    test('marshal and unmarshal IntValue', function() {
        for (const i of [0, 1, 100, '0x9271342394932492394']) {
            ...
            expect((unmarshaledValue as arb.IntValue).bignum.eq(bn(i))).toBe(
                true
            );
        }
    ...
    test('marshal and unmarshal CodePointValue', function() {
        ...
        const immTCV = new arb.CodePointValue(
            pc,
            new arb.ImmOp(0x19, iv),
            nextHash
        );
        ...
        expect(
            ((revImmValue.op as arb.ImmOp)
                .value as arb.IntValue).bignum.toNumber()
        ).toBe(60);
...
describe('Integration', function() {
    test('sizedByteRangeToBytes and hexToSizedByteRange', function() {
        ...
        const myValue = new arb.TupleValue([
            ...
            new arb.IntValue(
                bn(
                    '0x90e130e5da79003b67479a3ed2caf5585e93ae6771de6cdec6d7641bd2e60180'
                )
            )
        ]);
        ...
        const sizedByteRange = new arb.TupleValue([
            val,
            new arb.IntValue(bn(100))
        ]);
