...
describe('ArbConversion', () => {
    it('uses expected default values with no constructor args', () => {
        expect(defaultConverter.ticksPerBlock).toEqual(
            defaultVals.ticksPerBlock
        );
    ...
    it('converts correctly', () => {
        ...
        expect(defaultConverter.ticksToSeconds(randBN)).toEqual(
            randBN
                .div(defaultVals.ticksPerBlock)
                .mul(defaultVals.secondsPerBlock)
        );
        ...
        expect(defaultConverter.secondsToTicks(randBN)).toEqual(
            randBN
                .div(defaultVals.secondsPerBlock)
                .mul(defaultVals.ticksPerBlock)
        );
