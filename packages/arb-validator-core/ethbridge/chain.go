...
func WaitForReceiptWithResults(ctx context.Context, client ethutils.EthClient, from ethcommon.Address, tx *types.Transaction, methodName string) (*types.Receipt, error) {
	receipt, err := WaitForReceiptSimple(ctx, client, tx.Hash())
	if err != nil {
		return nil, err
	}
	if receipt.Status != 1 {
...
func WaitForReceiptSimple(ctx context.Context, client ethutils.EthClient, txHash ethcommon.Hash) (*types.Receipt, error) {
	for {
		select {
