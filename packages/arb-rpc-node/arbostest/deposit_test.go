...
func TestFailedERC20Deposit(t *testing.T) {
	tx1 := makeSimpleConstructorTx(hexutil.MustDecode(arbostestcontracts.FailedERC20Bin), big.NewInt(0))
	deposit := message.ERC20{
		Token: connAddress1,
		Dest:  sender,
	...
	logs, sends, _, _ := runAssertion(t, makeSimpleInbox(messages), 2, 1)
	results := processTxResults(t, logs)
	checkConstructorResult(t, results[0], connAddress1)
	...
	if outEthMsg.Value.Cmp(deposit.Value) != 0 {
		t.Fatal("wrong withdraw value", outEthMsg.Value)
	}

	if outEthMsg.Dest != deposit.Dest {
		t.Fatal("wrong withdraw destination", outEthMsg.Dest)
	}

	if outEthMsg.Token != deposit.Token {
		t.Fatal("wrong token address")
	}
}

func TestFailedERC721Deposit(t *testing.T) {
	tx1 := makeSimpleConstructorTx(hexutil.MustDecode(arbostestcontracts.FailedERC721Bin), big.NewInt(0))
	deposit := message.ERC721{
		Token: connAddress1,
		Dest:  sender,
		ID:    big.NewInt(100),
	}
	...
	outEthMsg := message.NewERC721FromData(outMsg.Data)

	if outEthMsg.ID.Cmp(deposit.ID) != 0 {
		t.Fatal("wrong withdraw id", outEthMsg.ID)
	}
