...
var logger = arblog.Logger.With().Str("component", "arb-dev-node").Logger()
...
func startup() error {
	...
	arbosUpgradePath := fs.String("arbos", "", "ArbOS version")
	...
	backend, db, cancelDevNode, txDBErrChan, err := dev.NewForkNode(
		ctx,
		*dbDir,
		chainId,
		agg,
