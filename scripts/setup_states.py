...
def setup_validator_states_ethbridge(contract, n_validators, sudo):
    ...
    layer = run('docker create arb-bridge-eth', capture_stdout=True, quiet=True, sudo=sudo).strip()
    if layer == '':
        print('Docker image arb-bridge-eth does not exist')
        return
    run('docker cp %s:/home/user/bridge_eth_addresses.json %s' % (layer, ethaddrs), sudo=sudo)
...
def main():
    ...
    parser.add_argument('contract', type=check_file,
        help='The Arbitrum bytecode contract to deploy')
    parser.add_argument('n_validators', type=check_range_2_100,
        help='The number of validators to deploy')
    parser.add_argument('-a', '--acctKeys', type=check_json,
                        default='./keys.json',
                        help='Generate with: ganache-cli --acctKeys keys.json -m "$MNEMONIC" -a "$NUM_WALLETS"')
    parser.add_argument('-b', '--bridge_eth_addresses', type=check_json,
    ...
    setup_validator_states(args.contract, args.n_validators, arb.acctKeys,
