...
- Here is an example of how to run nitro-node for goerli:
  ...
  ```
  docker run --rm -it  -v /some/local/dir/arbitrum:/home/user/.arbitrum -p 0.0.0.0:8547:8547 -p 0.0.0.0:8548:8548 offchainlabs/nitro-node:v2.0.0-beta.8-5ed2c72 --l1.url https://l1-goerli-node:8545 --l2.chain-id=421613 --http.api=net,web3,eth,debug --http.corsdomain=* --http.addr=0.0.0.0 --http.vhosts=*
  ```
...
- Here is an example of how to run nitro-relay for goerli:
  ```
  docker run --rm -it -p 0.0.0.0:9642:9642 --entrypoint relay offchainlabs/nitro-node:v2.0.0-beta.8-5ed2c72 --node.feed.output.addr=0.0.0.0 --node.feed.input.url wss://goerli-rollup.arbitrum.io/feed
  ```
- Here is an example of how to run nitro-node for goerli with custom relay:
  ```
  docker run --rm -it  -v /some/local/dir/arbitrum:/home/user/.arbitrum -p 0.0.0.0:8547:8547 -p 0.0.0.0:8548:8548 offchainlabs/nitro-node:v2.0.0-beta.8-5ed2c72 --l1.url https://l1-goerli-node:8545 --l2.chain-id=421613 --http.api=net,web3,eth,debug --http.corsdomain=* --http.addr=0.0.0.0 --http.vhosts=* --feed.input.url ws://local-relay-address:9642
  ```
