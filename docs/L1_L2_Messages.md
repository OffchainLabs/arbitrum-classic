...
Note that the first execution of the L2 transaction happens automatically, and in the happy / most-common case, it will simply succeed right then and there; thus, a user will typically have have to sign and publish a single transaction.
...
Additionally, the retryable ticket system safely accounts for cases where a user _overpays_ for the ArbGas required for the L2 transaction; it handles this by simply refunding the user excess ArbGas at their specified address.
