...
class IntValue extends Value {
    ...
    hash() {
        let bytes32 = utils.padZeros(utils.arrayify(this.bignum), 32);
...
class TupleValue extends Value {
    ...
    constructor(contents) {
        ...
        // Calculate the hash
        let hashes = contents.map((value, _) => value.hash());
        let types = ['uint8'].concat(Array(contents.length).fill('bytes32'));
        let hash = utils.soliditySha256(types, [TYPE_TUPLE_0 + contents.length].concat(hashes));
        this.hash = () => hash;
    }
    ...
    set(index, value) {
        ...
        let tuple = new TupleValue([...this.contents]);
        tuple.contents[index] = value;
        return tuple;
    }
}
...
function hexToSizedByteRange(hexString) {
    ...
    for (let i = utils.bigNumberify(0); i.lt(size); i = i.add(1)) {
        t = setBigTuple(t, i, utils.bigNumberify('0x' + h.slice(i*32*2, i*32*2 + 32*2)));
    }
    return t
}

function marshal(someValue) {
    return marshalValue('0x', someValue);
}

function marshalValue(acc, v) {
    let ty = v.typeCode();
    ...
    if (ty === TYPE_INT) {
        ...
        if (v.op.immCount === BASIC_OP_IMM_COUNT) {
            ...
            return marshalValue(packed, v.op.val) + v.nextHash.slice(2);
        }
        ...
        for (let i = 0; i < v.contents.length; i++) {
            accTy = marshalValue(accTy, v.contents[i]);
        }
        return accTy;
    }
}
...
function unmarshal(hexString) {
    return unmarshalValue(hexString)[0];
}
