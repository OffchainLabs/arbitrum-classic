...
// TODO: test ethers function in node?
...
class Value {
    ...
    internalTypeCode() { throw "unimplemented" }
    clone() { throw "unimplemented" }
    cloneShallow() { throw "unimplemented" }
    equal() { throw "unimplemented" }
}
...
class IntValue extends Value {
    ...
    hash() {
        // TODO: check this conversion works as expected
...
class CodePointValue extends Value {
    ...
    constructor(insnNum, op, nextHash) {
        ...
        // TODO: HaltCodePoint and ErrorCodePoint needed?
    }
...
class SizedTupleValue extends Value {
    ...
    constructor(contents, size) {
        ...
        this.contents = contents; // array of Values
        this.itemCount = size; // int8
        this.cachedHash = ; // 32 bytes
        // interface
        this.typeCode = () => TYPE_TUPLE_0;
        this.internalTypeCode = () => (TYPE_TUPLE_0 + size);
        // // let range = Array(size).fill().map((_, i) => i);
        // this.size = contents.reduce(((acc, value) => acc + value.size()), 1); // int64
        let hash = function() {
            let hashes = this.contents.reduce((acc, value) => acc.concat(value.hash()), "");
            // TODO: missing the right pad of bytes. Why would we need this if all 32B?
            let hashesF = "0x".concat(hashes.replace(/0x/g, '')); // TODO: formatting
            return utils.soliditySha256(['uint8', 'bytes' + (this.itemCount * 32)],
                                        [this.internalTypeCode(), hashesF]);
        }();
        this.hash = () => hash;
    }
}

// HexString should be binary data
function unmarshalValue(hexString) {
    ...
    if (ty === TYPE_INT) {
        ...
        let [head, tail] = extractBytes(tail, 32);
        let nextHash = head;
        return [CodePointValue(pc, op, nextHash), tail]; // TODO: CodePointValue
        ...
        let [head, tail] = extractBytes(tail, 32);
        let hash = parseInt(head, 16);
        return [HashOnlyValue(hash, size), tail]; // TODO: HashOnlyValue
    } else if ((ty - TYPE_TUPLE_0) >= TYPE_TUPLE_0 && ty <= TYPE_TUPLE_MAX) {
        let size = ty - TYPE_TUPLE_0;
        let [contents, tail] = unmarshalTuple(tail, size);
        return [SizedTupleValue(contents, size), tail]; // TODO: SizedTupleValue
    } else {
...
function unmarshalOp(hexString) {
    ...
    if (immCount == 0) {
        ...
        return [BasicOp(opcode), tail] // TODO: BasicOp and ImmOp
    } else if (immCount == 1) {
        let [opcode, tail] = unmarshalOpCode(tail);
