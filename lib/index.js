...
function processLogProof(value, logHash, proof) {
  if (proof.length === 0) {
    return false;
  }

  const kh = (t, v) => ethers.utils.solidityKeccak256(t, v);
  const valHash = value.hash();
  const startHash = proof[0];
  proof[0] = valHash;
  return (logHash === proof.reduce(
...
class ArbClient {
  ...
  async getMessageResult(txHash) {
    ...
    if (result["found"]) {
      ...
      let logHash = ArbValue.unmarshal(result["logHash"]);
      let logProofs = ArbValue.unmarshal(result["logProofs"]);

      // Step 1: prove that val is in logHash
      if (!processLogProof(val, logHash, logProofs)) {
