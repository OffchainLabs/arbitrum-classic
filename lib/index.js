...
// EthBridge VMCreated Event constants
const VMC_NAME = "VMCreated";
const VMC_ADDRESSES_INDEX = 8;
...
class ArbClient {
  ...
  getVMCreatedEventTxHash() {
    let self = this;
    return new Promise(function(resolve, reject) {
      self.client.request(
        "Validator.GetVMCreatedEventTxHash",
        [],
        function(err, error, result) {
          if (err) {
            reject(err);
            ...
            resolve(result["unanVMCreatedEventTxHash"]);
          }
        }
      )
    });
...
class ArbProvider extends ethers.providers.BaseProvider {
  ...
  async getValidatorAddresses() {
    if (!this.validatorAddresses) {
      let eventTxHash = await this.client.getVMCreatedEventTxHash()
      ...
      let events = await this.provider.waitForTransaction(eventTxHash)
          .then((receipt) =>
                ...
                (err) => { console.error(err); return null });
      let vmCreatedEvent = events.find((event) => event["name"] === VMC_NAME);
      if (vmCreatedEvent) {
        this.validatorAddresses = vmCreatedEvent["values"][VMC_ADDRESSES_INDEX]
  ...
  perform(method, params) {
    ...
    case "getCode":
      ...
      break;
    case "getVMCreatedEventTxHash":
      return this.client.getVMCreatedEventTxHash();
    case "getBlockNumber":
