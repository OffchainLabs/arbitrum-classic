...
function processLogProof(value, logPreHash, logPostHash, logValHashes) {
  ...
  /* DEBUG
  console.table({
    "functionName": "processLogProof",
    "value.hash()": value.hash(),
    "logPreHash": logPreHash,
    "logPostHash": logPostHash,
...
class ArbClient {
  ...
  async getMessageResult(txHash) {
    ...
    if (result["found"]) {
      ...
      let logPreHash = result["logPreHash"];
      let logPostHash = result["logPostHash"];
      let logValHashes = result["logValHashes"];

      // Step 1: prove that val is in logHash
      if (!processLogProof(val, logPreHash, logPostHash, logValHashes)) {
        console.log("value not in logHash!!!");
        return null; // TODO: error message? "val not in logHash?"
      }
      console.log("value is in the logHash as expected");
      // Step 2: prove that logHash is in assertion
      // Step 3: prove that assertion is valid
