...
class ArbProvider extends ethers.providers.BaseProvider {
  ...
  processUnanimousAssertion(partialHash, logPostHash, validatorSigs) {
    if (this.validatorAddresses.length !== validatorSigs.length) {
      console.error("Expected:", this.validatorAddresses.length, "signatures.\n",
                    "Received:", validatorSigs.length);
      return false;
    }

    let assertionHash = ethers.utils.solidityKeccak256(
      ["bytes32", "bytes32", "bytes32"], [this.vmId, partialHash, logPostHash]);
    ...
    for (let i = 0; i < this.validatorAddresses; i++) {
      if (this.validatorAddresses[i] !== addresses[i]) {
  ...
  async processConfirmedDisputableAssertion(logPostHash, onChainTxHash) {
    let receipt = await provider.waitForTransaction(onChainTxHash);
    let events = receipt.logs.map((l) => self.vmTracker.interface.parseLog(l));
    // DisputableAssertion Event
    let cda = events.find((event) => event["name"] === EB_EVENT_CDA);
    if (cda) {
      // Check correct VM
      if (cda.values.vmId !== self.vmId) {
    ...
    if (!this.validatorAddresses) {
      ...
      if (vmCreatedEvent) {
        ...
        if (this.vmId && vmCreatedEvent.values.vmId !== this.vmId) {
          ...
          return this.validatorAddresses.sort();
        } else {
  ...
  async getMessageResult(txHash) {
    // Fetch VMCreatedEvent vmId and validatorAddresses if not cached
    ...
    // Check vmId matches
    if (this.vmId !== vmId) {
      console.error("Incorrect VM ID:", vmId, "\nExpected:", this.vmId);
      return null;
    }
