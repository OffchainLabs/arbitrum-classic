...
// Returns true if the hash of value is in logHash and false otherwise
function processLogProof(value, logPreHash, logPostHash, logValHashes) {
...
// Returns all validator addresses as a []hexString
function getValidatorAddresses() {
  /*
  // Check if cached. Otherwise:
  // Watch for VMCreated
  vmTracker.on(filter, (from, to, value) => {
  });
  // get address[] validators

  // Lookup list of validator addresses
  provider.on({
    address contractEnsName,
    topics: [ ethers.utils.id("VMCreated()") ]
  }, (result) => {
    console.log(result);
  });
  */
...
function processUnanimousAssertion(partialHash, logPostHash, validatorSigs,
                                   validatorAddresses) {
  ...
  for (let sig of validatorSigs) {
    let address = ethers.utils.verifyMessage(assertionHash, sig);
    if (!addresses.includes(address)) {
      console.error("Invalid Unanimous Assertion signature: ", sig,
...
function processDisputableAssertion(partialHash, logPostHash, onChainTxHash,
                                    provider, contract) {
  provider.waitForTransaction(onChainTxHash).then((receipt) => {
    logData = contract.interface.parseLog(receipt.logs[0]);
    // TODO
    console.log(logData);
...
class ArbProvider extends ethers.providers.BaseProvider {
  ...
  async getMessageResult(txHash) {
    ...
    // Step 1: prove that val is in logHash
    if (!processLogProof(val, logPreHash, logPostHash, logValHashes)) {
      console.error("Failed to prove val is in logPostHash");
      return null;
    }

    // Step 2: prove that logHash is in assertion and assertion is valid
    if (validatorSigs.length > 0) {
      let filter = this.vmTracker.filters.VMCreated;
      // TODO: hard coding these
      let validatorAddresses = ['0a7a17713f8ba8d4e91e65af61eb6e480745b2aa',
                                '7d677c5191aeaf4e4866728d6db8e0799cc0a179',
                                'a5e9901442f9dbbf028abaa5eb1e2b81f4447c32',
                               ];
      /*
      validatorAddresses = this.vmTracker.on(filter, (from, to, value) => {
        console.log("");
      });
      */
      if (!processUnanimousAssertion(partialHash, logPostHash, validatorSigs,
                                     validatorAddresses)) {
        return null;
      }
    } else {
      if (!processDisputableAssertion(partialHash, logPostHash, onChainTxHash,
                                      this.provider)) {
        return null;
      }
    }
